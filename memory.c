...
{
	return (9
		+ ((_libhypervplat_cfg.initrd.len > 0) ? 1 : 0)
		+ ((_libhypervplat_cfg.heap2.len  > 0) ? 1 : 0));
}
...
int ukplat_memregion_get(int i, struct ukplat_memregion_desc *m)
{
	...
	case 6: /* bss */
		...
		break;
	case 7: /* heap */
		m->base  = (void *) _libhypervplat_cfg.heap.start;
		m->len   = _libhypervplat_cfg.heap.len;
		...
		break;
	case 8: /* stack */
		m->base  = (void *) _libhypervplat_cfg.bstack.start;
		m->len   = _libhypervplat_cfg.bstack.len;
		m->flags = (UKPLAT_MEMRF_RESERVED
			    | UKPLAT_MEMRF_READABLE
			    | UKPLAT_MEMRF_WRITABLE);
		ret = 0;
	...
	case 9: /* initrd */
		if (_libhypervplat_cfg.initrd.len) {
			m->base  = (void *) _libhypervplat_cfg.initrd.start;
			m->len   = _libhypervplat_cfg.initrd.len;
	...
	case 10: /* heap2
		...
		if (_libhypervplat_cfg.initrd.len && _libhypervplat_cfg.heap2.len) {
			m->base  = (void *) _libhypervplat_cfg.heap2.start;
			m->len   = _libhypervplat_cfg.heap2.len;
			m->flags = UKPLAT_MEMRF_ALLOCATABLE;
#if CONFIG_UKPLAT_MEMRNAME
			m->name  = "heap";
#endif
