...
#include <x86/cpu.h>
#include <x86/traps.h>
#include <hyperv/config.h>
#include <hyperv/console.h>
#include <hyperv/intctrl.h>
#ifndef CONFIG_UKPLAT_HAVE_MULTIBOOT2
#include <hyperv-x86/multiboot.h>
#include <hyperv-x86/multiboot_defs.h>
#else
#include <hyperv-x86/multiboot2.h>
...
struct hypervplat_config _libhypervplat_cfg = {0};

extern void _libhypervplat_newstack(uintptr_t stack_start, void (*tramp)(void *),
				 void *arg);
...
static inline void _mb_init_mem(struct multiboot_tag *tag)
{
	...
	_libhypervplat_cfg.heap.start = ALIGN_UP((uintptr_t)__END, __PAGE_SIZE);
	_libhypervplat_cfg.heap.end = (uintptr_t)max_addr - __STACK_SIZE;
	_libhypervplat_cfg.heap.len =
	    _libhypervplat_cfg.heap.end - _libhypervplat_cfg.heap.start;
	_libhypervplat_cfg.bstack.start = _libhypervplat_cfg.heap.end;
	_libhypervplat_cfg.bstack.end = max_addr;
	_libhypervplat_cfg.bstack.len = __STACK_SIZE;
}
...
static inline void _mb_init_initrd(struct multiboot_tag_module *mod1)
{
	...
	_libhypervplat_cfg.initrd.start = (uintptr_t)mod1->mod_start;
	_libhypervplat_cfg.initrd.end = (uintptr_t)mod1->mod_end;
	_libhypervplat_cfg.initrd.len = (size_t)(mod1->mod_end - mod1->mod_start);

	/*
	 * Check if initrd is part of heap
	 * In such a case, we figure out the remaining pieces as heap
	 */
	if (_libhypervplat_cfg.heap.len == 0) {
	...
	heap1_start = 0;
	heap1_end = 0;
	if (RANGE_OVERLAP(_libhypervplat_cfg.heap.start, _libhypervplat_cfg.heap.len,
			  _libhypervplat_cfg.initrd.start,
			  _libhypervplat_cfg.initrd.len)) {
		if (IN_RANGE(_libhypervplat_cfg.initrd.start,
			     _libhypervplat_cfg.heap.start,
			     _libhypervplat_cfg.heap.len)) {
			/* Start of initrd within heap range;
			 * Use the prepending left piece as heap */
			heap0_start = _libhypervplat_cfg.heap.start;
			heap0_end = ALIGN_DOWN(_libhypervplat_cfg.initrd.start,
					       __PAGE_SIZE);
		}
		if (IN_RANGE(_libhypervplat_cfg.initrd.start,
			...
			heap1_start =
			    ALIGN_UP(_libhypervplat_cfg.initrd.end, __PAGE_SIZE);
			heap1_end = _libhypervplat_cfg.heap.end;
		}
	} else {
		/* Initrd is not overlapping with heap */
		heap0_start = _libhypervplat_cfg.heap.start;
		heap0_end = _libhypervplat_cfg.heap.end;
	}
	 ...
	 * `_libhypervplat_cfg.heap`. Any additional piece will then be
	 * placed to `_libhypervplat_cfg.heap2`.
	 */
	if (heap0_len == 0) {
		/* Heap piece 0 is empty, use piece 1 as only */
		if (heap1_len != 0) {
			_libhypervplat_cfg.heap.start = heap1_start;
			_libhypervplat_cfg.heap.end = heap1_end;
			_libhypervplat_cfg.heap.len = heap1_len;
		} else {
			_libhypervplat_cfg.heap.start = 0;
			_libhypervplat_cfg.heap.end = 0;
			_libhypervplat_cfg.heap.len = 0;
		}
		_libhypervplat_cfg.heap2.start = 0;
		_libhypervplat_cfg.heap2.end = 0;
		_libhypervplat_cfg.heap2.len = 0;
	} else {
		/* Heap piece 0 has memory */
		_libhypervplat_cfg.heap.start = heap0_start;
		_libhypervplat_cfg.heap.end = heap0_end;
		_libhypervplat_cfg.heap.len = heap0_len;
		if (heap1_len != 0) {
			_libhypervplat_cfg.heap2.start = heap1_start;
			_libhypervplat_cfg.heap2.end = heap1_end;
			_libhypervplat_cfg.heap2.len = heap1_len;
		} else {
			_libhypervplat_cfg.heap2.start = 0;
			_libhypervplat_cfg.heap2.end = 0;
			_libhypervplat_cfg.heap2.len = 0;
...
no_initrd:
	_libhypervplat_cfg.initrd.start = 0;
	_libhypervplat_cfg.initrd.end = 0;
	_libhypervplat_cfg.initrd.len = 0;
	_libhypervplat_cfg.heap2.start = 0;
	_libhypervplat_cfg.heap2.end = 0;
	_libhypervplat_cfg.heap2.len = 0;
	return;
...
static void _libhypervplat_entry2(void *arg __attribute__((unused)))
{
	ukplat_entry_argp(NULL, cmdline, sizeof(cmdline));
}
#ifndef CONFIG_UKPLAT_HAVE_MULTIBOOT2
void _libhypervplat_entry(void *arg)
{
	struct multiboot_info *mi = (struct multiboot_info *)arg;
#else
void _libhypervplat_entry(unsigned long mi)
{
	...
	_init_cpufeatures();
	_libhypervplat_init_console();
	traps_init();
	intctrl_init();

	uk_pr_info("Entering from Hyper-V (x86)...\n");
	...
	if (_libhypervplat_cfg.initrd.len)
		uk_pr_info("        initrd: %p\n",
			   (void *)_libhypervplat_cfg.initrd.start);
	uk_pr_info("    heap start: %p\n", (void *)_libhypervplat_cfg.heap.start);
	if (_libhypervplat_cfg.heap2.len)
		uk_pr_info(" heap start (2): %p\n",
			   (void *)_libhypervplat_cfg.heap2.start);
	uk_pr_info("     stack top: %p\n",
		   (void *)_libhypervplat_cfg.bstack.start);
	...
	uk_pr_info("Switch from bootstrap stack to stack @%p\n",
		   (void *)_libhypervplat_cfg.bstack.end);
	_libhypervplat_newstack(_libhypervplat_cfg.bstack.end, _libhypervplat_entry2, 0);
}
